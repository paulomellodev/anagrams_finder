...
const alphabetize = (wordToAlphabetical) => {
...
const findAnagrams = (typedWordsAlphabetical, result, db) => {
  let output = result;
...
const filterWordsByLength = (size, db) => {
  let filteredWords = [];
  filteredWords = db.filter((word) => word.length === size);
  return filteredWords;
};

const longestWord = (db) => {
  let maxLength = 0;
